...
- Spark purchase amounts
- Order/checkout IDs, transaction status, product IDs
- External user ID (Clerk ID) to credit your account
- Payment details are processed by our payment providers; we do not store card numbers or crypto wallet details
...
- **Provide the Services**: Persona creation, chat, image generation, spark management
- **Authenticate and authorize**: Manage your account and access
- **Process payments**: Handle purchases, subscriptions, and spark credits
...
If you request help with a specific issue (e.g., failed image generation, spark refund):
...
- Usage metadata (spark consumption, model ID, approximate costs)
...
| ------------------- | ------------------------ | ------------------------------------------------------------------------------- |
| **Neon** (Database) | Data storage             | User IDs, personas, chat history, images metadata, spark balances, transactions |
...
- Processed by our payment processors and cryptocurrency providers
- We store: order IDs, product IDs, spark amounts, transaction status, your Clerk ID
- **We do not store**: Credit card numbers, bank details, or crypto wallet private keys
- Payment providers process sensitive payment data per their compliance standards (PCI-DSS for cards)
...
| Payment processors & crypto providers | Payment processing          | Varies               |
...
| **Spark transactions**       | Up to 7 years (accounting and tax compliance)       |
...
4. Transaction records are retained for legal/accounting requirements (anonymized where possible)
