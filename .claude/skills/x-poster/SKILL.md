...
The API key is stored in `.env.local` at the project root as `OPENTWEET_API_KEY`.

**Never read, log, or display the key value.** Always load it by sourcing `.env.local` in the same shell command so the shell handles substitution and the key is never exposed in conversation output.

## Authentication

Every curl request must be prefixed with `source .env.local &&` so the key is injected by the shell at runtime:
...
For file uploads, replace `-H "Content-Type: application/json"` with `-F "file=@your-file.png"` (curl sets multipart automatically).
